...
void eliminacionAdelante(float *AB, int n, int poscol) {
    for (int id = 0; id < n - 1 - poscol; ++id) {
        ...
        for (int i = 0; i < n; ++i) {
            if (i != poscol) {
                int posfactor = (n + 1) * i + poscol;
                float factor = AB[posfactor];

                for (int j = pospivot; j < posfinfila; ++j) {
                    int posactualelim = (n + 1) * i + j % (n + 1);
...
int main() {
    ...
    std::cout << "Average Sequential Time: " << sequential_time.count() * 1000 << " ms\n";
    std::cout << "  Average Parallel Time: " << parallel_time.count() * 1000 << " ms\n";
